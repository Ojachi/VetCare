...
export default function PublicProductsCatalog() {
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity style={styles.card} onPress={() => router.push({ pathname: '/(auth)/product-detail', params: { id: String(item.id) } } as any)}>
      ...
      {!user && <Text style={styles.loginHint}>Inicia sesión para comprar</Text>}
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  ...
  loginHint: { marginTop: 6, fontSize: 12, color: colors.secondary, fontWeight: '600' },
});

...
export default function VetHome() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      const vetAppointments = appointments.filter((a: any) => a.assignedTo?.id === user?.id);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[typography.h2, styles.greeting]}>Bienvenido, Dr. {user?.name}</Text>
        <Text style={styles.role}>Veterinario de VetCare</Text>
      </View>
...
const menuItems = [
  {
    ...
    icon: '📅',
    label: 'Mis Citas',
    description: 'Ver citas asignadas',
    path: '/(veterinarian)/vet-appointments',
  ...
  },
  {
    ...
    description: 'Llenar diagnóstico',
    path: '/(veterinarian)/register-diagnosis',
  ...
  },
  {
    ...
    path: '/(veterinarian)/patient-history',
